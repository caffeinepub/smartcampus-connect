...
import { Star, TrendingUp, BookOpen, Users, ChevronDown, ChevronUp, Crown, Shield } from 'lucide-react';
...
const mockFaculty: Faculty[] = [
  {
    id: 1, name: 'Dr. Rajesh Kumar', department: 'CSE', designation: 'Professor',
    teachingScore: 94, contributionIndex: 88, researchCount: 12, feedbackRating: 4.8,
    overallScore: 92, avatar: 'RK',
    improvements: ['Increase industry collaboration', 'Mentor more student projects'],
  },
  {
    id: 2, name: 'Prof. Anita Sharma', department: 'IT', designation: 'Associate Professor',
    teachingScore: 89, contributionIndex: 82, researchCount: 8, feedbackRating: 4.6,
    overallScore: 87, avatar: 'AS',
    improvements: ['Publish more research papers', 'Attend national conferences'],
  },
  {
    id: 3, name: 'Dr. Suresh Nair', department: 'ECE', designation: 'Professor',
    teachingScore: 91, contributionIndex: 90, researchCount: 15, feedbackRating: 4.7,
    overallScore: 91, avatar: 'SN',
    improvements: ['Improve student engagement scores', 'Lead more workshops'],
  },
  {
    id: 4, name: 'Prof. Meena Patel', department: 'Mechanical', designation: 'Assistant Professor',
    teachingScore: 78, contributionIndex: 65, researchCount: 4, feedbackRating: 4.2,
    overallScore: 72, avatar: 'MP',
    improvements: ['Increase research publications', 'Improve assignment feedback turnaround', 'Participate in FDPs'],
  },
  {
    id: 5, name: 'Dr. Vikram Joshi', department: 'Civil', designation: 'Professor',
    teachingScore: 86, contributionIndex: 79, researchCount: 10, feedbackRating: 4.5,
    overallScore: 84, avatar: 'VJ',
    improvements: ['Increase hackathon mentorship', 'Contribute to curriculum design'],
  },
  {
    id: 6, name: 'Prof. Kavitha Reddy', department: 'EEE', designation: 'Associate Professor',
    teachingScore: 82, contributionIndex: 74, researchCount: 6, feedbackRating: 4.3,
    overallScore: 79, avatar: 'KR',
    improvements: ['Publish in indexed journals', 'Increase student project guidance'],
  },
];
...
export default function FacultyNetworking() {
  ...
  const ranked = [...mockFaculty].sort((a, b) => b.overallScore - a.overallScore);
          ...
          {ranked.map((f, i) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-fhub-heading truncate">{f.name}</p>
                <p className="text-xs text-fhub-muted">{f.department} · {f.designation}</p>
              </div>
              <div className="flex items-center gap-4 flex-shrink-0">
                <div className="text-right hidden sm:block">
                  <p className="text-xs text-fhub-muted">Teaching</p>
                  <p className="text-sm font-bold text-fhub-accent">{f.teachingScore}</p>
                </div>
                <div className="text-right hidden md:block">
                  <p className="text-xs text-fhub-muted">Research</p>
                  <p className="text-sm font-bold text-fhub-accent">{f.researchCount} papers</p>
                </div>
                <div className="text-right">
                  <p className="text-xs text-fhub-muted">Overall</p>
                  <p className="text-lg font-display font-bold text-fhub-accent">{f.overallScore}</p>
                </div>
              </div>
            </div>
          ))}
          ...
          return (
            <div key={faculty.id} className="bg-white dark:bg-fhub-card rounded-2xl border border-fhub-border shadow-fhub overflow-hidden">
              ...
              <div className="p-4 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-2.5 rounded-xl bg-fhub-bg">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <span className="text-[10px] text-fhub-muted">Teaching Score</span>
                    </div>
                  ...
                  <div className="p-2.5 rounded-xl bg-fhub-bg">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <span className="text-[10px] text-fhub-muted">Research Papers</span>
                    </div>
                    <p className="text-lg font-bold text-fhub-accent">{faculty.researchCount}</p>
                  </div>
                  <div className="p-2.5 rounded-xl bg-fhub-bg">
                    <div className="flex flex-col gap-1">
                      <span className="text-[10px] text-fhub-muted">Student Feedback</span>
                ...
                >
                  <span className="text-xs font-medium text-fhub-heading">Suggestions to Improve</span>
                  {isExpanded ? (
