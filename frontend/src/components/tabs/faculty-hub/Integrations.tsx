...
import { Monitor, FileText, Github, Linkedin, RefreshCw, FileOutput, CheckCircle2, XCircle, Clock } from 'lucide-react';
...
export default function Integrations() {
  ...
  const handleAction = (id: string) => {
    if (id === 'smartboard') {
      ...
      setDetails(prev => ({ ...prev, [id]: `Last generated: ${new Date().toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}` }));
    }
  };
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-6 py-8">
      <div className="mb-6">
        <h2 className="font-display font-bold text-fhub-heading text-xl">Connected Integrations</h2>
        <p className="text-sm text-fhub-muted mt-1">Manage your external tools and platform connections</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {integrations.map((integration) => {
          ...
          return (
            <div key={integration.id} className="bg-white dark:bg-fhub-card rounded-2xl border border-fhub-border shadow-fhub p-6 flex flex-col gap-4">
              {/* Header */}
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-semibold text-fhub-heading text-sm">{integration.name}</h3>
                    <p className="text-xs text-fhub-muted leading-tight mt-0.5">{integration.description}</p>
                  </div>
                </div>
      ...
      <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Active Integrations', value: Object.values(statuses).filter(s => s === 'connected').length, color: 'text-green-600' },
          { label: 'Pending Setup', value: Object.values(statuses).filter(s => s === 'disconnected').length, color: 'text-orange-500' },
          { label: 'Total Platforms', value: integrations.length, color: 'text-fhub-accent' },
        ].map((stat) => (
