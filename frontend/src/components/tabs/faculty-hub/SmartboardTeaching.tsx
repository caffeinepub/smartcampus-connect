...
const subjectsByBranch: Record<string, string[]> = {
  ...
  IT: ['Web Technologies', 'Cloud Computing', 'Cybersecurity', 'Mobile App Dev', 'Data Mining'],
  ECE: ['Digital Electronics', 'Signals & Systems', 'VLSI Design', 'Embedded Systems', 'Communication Systems'],
  ...
  EEE: ['Power Systems', 'Control Systems', 'Electric Machines', 'Power Electronics', 'Renewable Energy'],
};
...
export default function SmartboardTeaching() {
  ...
  const [topics, setTopics] = useState(topicsBySubject['Data Structures'] || defaultTopics);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-white dark:bg-fhub-card rounded-2xl border border-fhub-border shadow-fhub p-6">
          ...
          <div className="rounded-xl bg-fhub-bg border border-fhub-border p-4 mb-4 flex items-center justify-center min-h-[120px]">
            <div className="text-center">
              ...
              <p className="text-xs text-green-600 font-medium mt-1">● Live Sync Active</p>
            </div>
          </div>
          <div className="space-y-2 text-xs text-fhub-muted">
            <div className="flex justify-between">
              <span>Resolution</span><span className="font-medium text-fhub-heading">4K UHD</span>
            </div>
            <div className="flex justify-between">
              <span>Touch Points</span><span className="font-medium text-fhub-heading">20-point</span>
            </div>
            <div className="flex justify-between">
              <span>Sync Status</span><span className="font-medium text-green-600">Real-time</span>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-fhub-card rounded-2xl border border-fhub-border shadow-fhub p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div>
              <Label className="text-xs text-fhub-muted mb-1.5 block">Branch</Label>
            ...
            <div>
              <Label className="text-xs text-fhub-muted mb-1.5 block">Division</Label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-fhub-card rounded-2xl border border-fhub-border shadow-fhub p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-fhub-heading text-sm">Notes Auto-Save</h3>
          </div>
          <div className="flex items-center gap-2 mb-2">
            <CheckCircle2 className="w-4 h-4 text-green-500" />
            <span className="text-xs text-fhub-muted">Saved automatically</span>
          </div>
          <p className="text-xs text-fhub-muted">Last saved: <span className="text-fhub-heading font-medium">{lastSaved}</span></p>
          <div className="mt-3 flex items-center justify-between">
            <Label className="text-xs text-fhub-muted">Auto-Save</Label>
            <Switch defaultChecked className="data-[state=checked]:bg-fhub-accent" />
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-fhub-card rounded-2xl border border-fhub-border shadow-fhub p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-fhub-heading text-sm">Lecture Recording</h3>
          </div>
          <Button
            variant="outline"
            onClick={() => setRecording(!recording)}
            className={`w-full text-xs h-9 rounded-lg border-fhub-border ${
              recording ? 'bg-red-50 text-red-600 border-red-200' : 'text-fhub-heading'
            }`}
          >
            {recording ? (
        ...
        <div className="bg-white dark:bg-fhub-card rounded-2xl border border-fhub-border shadow-fhub p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-semibold text-fhub-heading text-sm">Auto-Upload to Portal</h3>
          </div>
          ...
          <p className="text-xs text-fhub-muted">
            Status:{' '}
            <span className={`font-medium ${autoUpload ? 'text-green-600' : 'text-orange-500'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-fhub-card rounded-2xl border border-fhub-border shadow-fhub p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-fhub-muted">{completedCount}/{topics.length} completed</span>
          </div>
          <Progress value={completionPct} className="h-2 mb-4 bg-fhub-bg [&>div]:bg-fhub-accent" />
          <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
            {topics.map((topic, i) => (
              ...
              >
                ...
                <span className={`text-sm ${topic.completed ? 'text-fhub-muted line-through' : 'text-fhub-heading'}`}>
                  {topic.name}
                </span>
                {!topic.completed && i === topics.findIndex((t) => !t.completed) && (
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white dark:bg-fhub-card rounded-2xl border border-fhub-border shadow-fhub p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-fhub-heading text-sm">Last Topic Covered</h3>
            </div>
            {lastCovered ? (
              <div className="px-3 py-2 rounded-lg bg-fhub-badge-bg border border-fhub-accent/20">
                <p className="text-sm font-medium text-fhub-accent">{lastCovered.name}</p>
                <p className="text-xs text-fhub-muted mt-0.5">Completed · {subject}</p>
              </div>
            ) : (
          ...
          {nextTopic && (
            <div className="bg-fhub-accent/5 border border-fhub-accent/20 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="font-semibold text-fhub-heading text-sm">Resume From Here</h3>
              </div>
              <p className="text-xs text-fhub-muted mb-3">Suggested next topic:</p>
              <p className="text-sm font-semibold text-fhub-accent mb-3">{nextTopic.name}</p>
              <Button
                size="sm"
